...
function Step3Report({ report }) {
  ...
  const convertToTen = (score) => {
  if (!score) return 0;

  let result = score <= 10 ? score : (score / 25) * 10;
  ...
  const rawFinal = report.finalScore || 0;
  
  const questionWiseScore = report.questionWiseScore || [];

  
  const calculatedFinalScore = questionWiseScore.length
  ? questionWiseScore.reduce((sum, q) => {
      return sum + convertToTen(q.score || 0);
    }, 0) / questionWiseScore.length
  : 0;

const finalScore = round(Math.min(calculatedFinalScore, 10));
  const confidence =
  questionWiseScore.length
    ? round(
        questionWiseScore.reduce((sum, q) => {
          return sum + convertToTen(q.confidence || 0);
        }, 0) / questionWiseScore.length
      )
    : 0;
...
const communication =
  questionWiseScore.length
    ? round(
        questionWiseScore.reduce((sum, q) => {
          return sum + convertToTen(q.communication || 0);
        }, 0) / questionWiseScore.length
      )
    : 0;
...
const correctness =
  questionWiseScore.length
    ? round(
        questionWiseScore.reduce((sum, q) => {
          return sum + convertToTen(q.correctness || 0);
        }, 0) / questionWiseScore.length
      )
    : 0;
  ...
  const questionScoreData = questionWiseScore.map((q, index) => ({
    name: `Q${index + 1}`,
    score: round(convertToTen(q.score || 0))
  }));
  const skills = [
  ...
  const downloadPDF = () => {
    ...
    if (finalScore >= 8) {
      ...
      advice =
        "Significant imporvement required. Focus on structured thinking, clarity, and confident delivery. Practice answering aloud regularly.";
    }
    ...
    autoTable(doc, {
      ...
      body: questionWiseScore.map((q, i) => [
        `${i + 1}`,
        q.question,
        `${round(convertToTen(q.score || 0))}/10`,
        q.feedback,
      ]),
      styles: {
        fontSize: 9,
    ...
    doc.save("AI_Interview_Report.pdf"); 3
  }
    ...
    <div className='min-h-screen bg-gradient-to-br from-gray-50 to-green-50 px-4 sm:px-6 lg:px-10 py-8'>
      ...
      <div className='mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
        ...
        <button
          ...
          text-white py-3 px-6 rounded-xl shadow-md transition-all duration-300 
                ...
                sm:rounded-2xl border border-gray-200'>
                  ...
                  sm:items-start gap-3 mb-4'>
                    <div>
                      ...
                      <p className='font-semibold text-xs text-gray-800 sm:text-base
                      leading-relaxed'>
